...
class App(tk.Tk):
    def __init__(self):
        ...
        self._build_ui()
        self._bind_keys()
        # Start with the simplest complete template
        self._load_template(xml_io.template_helix_standard())
    ...
    def _build_menu(self):
        ...
        for name, factory in xml_io.TEMPLATES.items():
            tm.add_command(label=name,
                           command=lambda f=factory: self._ask_load_template(f))
    ...
    def action_new(self):
        ...
        self._update_title()
        self._set_status("New configuration — Helix Standard template loaded.")
    ...
    def _ask_load_template(self, factory):
        if self.modified:
            if not messagebox.askyesno("Load Template",
                                        "Unsaved changes will be lost. Continue?"):
                return
        self._load_template(factory())
        self.current_file = None
        self.modified = False
        self.undo_stack.clear()
        self._update_title()
        self._set_status("Template loaded.")
